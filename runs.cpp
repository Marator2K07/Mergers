...
void Runs::selectRunsReceiver(int a[],
                              ...
                              int &level)
{
    // в приоритете последовательность с наибольшим количеством
    // фиктивных серий
    if (d[j] < d[j+1]) {
        j++;
    } else {
        // если количество фиктивных серий закончилось, то
        // переходим на следующий уровень и вычисляем "строчку"
        // значений, фиктивных и реальных, причем - через фиктивное
        // число мы ставим себе цель на количество серий на данном уровне
        if (d[j] == 0) {
            level++;
            int temp = a[0];
    ...
    // таким образом помечаем, что заменили фиктивную серию
    // на реальную
    d[j]--;
}
